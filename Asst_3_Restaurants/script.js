...
data.forEach(function (r) {
  ...
  var result = r.inspection_results || "";
  var city = r.city || "Unknown";
  var type = r.inspection_type || "Unknown";

  if (result.includes("Violations") || result.includes("violations")) {
    failed = failed + 1;

    if (!byCity[city]) { byCity[city] = 0; }
    byCity[city] = byCity[city] + 1;
...
var passedCount = total - failed;


new Chart(document.getElementById("chart-fail-pass"), {
  type: "doughnut",
  data: {
    labels: ["Pass", "Fail"],
  datasets: [{ data: [passedCount, failed],
       backgroundColor: ["#152f59", "#8B1E1E"]
     }]
  },
...
//var cityLabels = Object.keys(Count_by_City);
//var cityCounts = Object.values(Count_by_City);
...
new Chart(document.getElementById("chart-by-city"), {
  ...
  labels: Object.keys(byCity),
    datasets: [{ label: "Failed inspections", data: Object.values(byCity),
...
new Chart(document.getElementById("chart-by-type"), {
  ...
  data: {
    ...
    datasets: [{ label: "Failed inspections",data: Object.values(byType),
      backgroundColor: "#8B1E1E"
     }]
  },
  options: 
